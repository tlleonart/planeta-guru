...
export interface ComboPageProps {
  product: Product;
}

/**
 * Server Component: Pagina principal de Combo
 * - Recibe el producto ya fetched
 * - Estructura de 3 secciones: Banner con card, Info detallada, Categorias relacionadas
...
export const ComboPage: FC<ComboPageProps> = ({ product }) => {
  ...
  return (
    <main>
      {/* Seccion 1: Banner con card */}
      <section className="h-full md:h-screen pt-16 md:pt-6">
        <ComboContainer product={product} variant="banner">
          <ComboCard
            ...
            bundleTitle={product.bundles[0]?.title || ""}
          />
        </ComboContainer>
      </section>

      {/* Seccion 2: Informacion detallada */}
      <section className="h-full mb-2 md:mb-4">
        <ComboContainer product={product} variant="info">
          {/* Grid de bundles (solo si hay multiples) */}
      ...
      {/* Seccion 3: Categorias relacionadas - TODO: Implementar cuando se migre el carousel */}
