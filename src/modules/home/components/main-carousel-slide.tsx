...
export const MainCarouselSlide: FC<MainCarouselSlideProps> = ({
  product,
  watchButtonLabel,
}) => {
  const {
    id,
    product: { media, name, descriptions, slug, productType },
  } = product;
    ...
    >
      <div className="flex flex-col md:block">
        ...
        <MainCarouselSlideCard
          name={name}
          description={
            descriptions && descriptions?.length > 0 ? descriptions[0].text : ""
          }
          slug={slug}
          type={
            productType?.name?.toLocaleLowerCase().replace(" ", "-") ||
            "game-html"
          }
